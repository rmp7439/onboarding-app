  ...
  const validateStep = useCallback(
    (step: number): boolean => {
      ...
      if (step === 1) {
        ...
        if (isReq('gender')) check("gender", formData.gender.length > 0, "Required");
        if (isReq('education')) check("highestEducation", formData.highestEducation.length > 0, "Required");
        if (isReq('maritalStatus')) check("maritalStatus", formData.maritalStatus.length > 0, "Required");

      } else if (step === 2) {
        if (isReq('aadhaar')) check("aadhaarNumber", formData.aadhaarNumber.trim().length > 0, "Required");
        if (isReq('pan')) check("panNumber", formData.panNumber.trim().length > 0, "Required");
        if (isReq('uan')) check("uanNumber", formData.uanNumber.trim().length > 0, "Required");
        if (isReq('esic')) check("esicNumber", formData.esicNumber.trim().length > 0, "Required");
        if (isReq('drivingLicence')) check("drivingLicence", formData.drivingLicence.trim().length > 0, "Required");
        ...
        if (isReq('accountHolderName')) check("accountHolderName", formData.accountHolderName.trim().length > 0, "Required");
        if (isReq('bankName')) check("bankName", formData.bankName.trim().length > 0, "Required");
        if (isReq('accountNumber')) check("accountNumber", formData.accountNumber.trim().length > 0, "Required");
        if (isReq('ifsc')) check("ifscCode", formData.ifscCode.trim().length > 0, "Required");
        if (isReq('micr')) check("micrCode", formData.micrCode.trim().length > 0, "Required");
        ...
        if (isReq('nomineeName')) check("nomineeName", formData.nomineeName.length > 0, "Required");
        if (isReq('nomineeRelation')) check("nomineeRelation", formData.nomineeRelation.length > 0, "Required");
        if (isReq('nomineeMobile')) check("nomineeMobile", formData.nomineeMobile.length === 10, "Must be 10 digits");
        if (isReq('nomineePercentage')) {
