...
export default function AadhaarUploadScreen() {
  ...
  useEffect(() => {
    const progressListener = progress.addListener(({ value }) => {
      ...
      const remainingSecs = Math.max(
        0,
        Math.ceil(
          ((1 - value) * (OCR_STAGES.length * ANIMATION.OCR_STAGE_DELAY_MS)) /
            1000,
        ),
      );
    ...
    try {
      ...
      if (result) {
        ...
        Animated.spring(previewAnim, {
          ...
          damping: ANIMATION.PREVIEW_SPRING_DAMPING,
          stiffness: ANIMATION.PREVIEW_SPRING_STIFFNESS,
        }).start();
      }
    } catch (error: any) {
      Alert.alert(
  ...
  const handleUseImage = () => {
    ...
    Animated.timing(previewAnim, {
      toValue: 0,
      duration: ANIMATION.PREVIEW_FADE_OUT_MS,
      useNativeDriver: true,
  ...
  const handleRetake = () => {
    ...
    Animated.timing(previewAnim, {
      toValue: 0,
      duration: ANIMATION.PREVIEW_RETAKE_FADE_OUT_MS,
      useNativeDriver: true,
    }).start(() => {
      setPreview(null);
      setTimeout(
        () => handlePickImage(side, source),
        ANIMATION.PREVIEW_RETAKE_DELAY_MS,
      );
    });
  };
  ...
  const handleContinue = async () => {
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(scanLineY, {
          toValue: ANIMATION.SCAN_LINE_TRAVEL_DISTANCE,
          duration: ANIMATION.SCAN_LINE_DURATION_MS,
          useNativeDriver: true,
        }),
        Animated.timing(scanLineY, {
          toValue: 0,
    ...
    Animated.loop(
      Animated.timing(spinnerAnim, {
        toValue: 1,
        duration: ANIMATION.SPINNER_DURATION_MS,
      ...
      if (step < OCR_STAGES.length) {
        Animated.sequence([
          Animated.timing(textOpacity, {
            toValue: 0,
            duration: ANIMATION.TEXT_FADE_OUT_MS,
            useNativeDriver: true,
          }),
        ]).start(() => {
          ...
          Animated.timing(textOpacity, {
            toValue: 1,
            duration: ANIMATION.TEXT_FADE_IN_MS,
            useNativeDriver: true,
          }).start();
        });
      } else {
    ...
    try {
      ...
      if (ocrResult.success) {
        const parsedData = await aadhaarParser.parse(ocrResult.rawText); // Using standard async API
  ...
  return (
    <Screen style={styles.container}>
      ...
      <OCRProcessingOverlay
        ...
        spinnerInterpolate={spinnerAnim.interpolate({
          inputRange: [0, 1],
